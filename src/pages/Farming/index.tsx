...
const Farming = () => {
  ...
  const { data: poolInfo, isLoading: poolInfoLoading } = useContractRead({
    abi: PoolAbi,
    address: searchParams.get('address') as `0x${string}`,
  ...
  const handleStakeOk = async () => {
    try {
      if (isConnected) {
        ...
        const txn = await PoolContract.stake(BigInt(stakeData.Amount * 10 * 1e8), { gasLimit: 1000000 });
  ...
  const handleBuyOk = async () => {
    try {
      if (isConnected) {
        ...
        const txn = await SlotContract.buySlots(data, BigInt(Number(buyAmount) * 10 * 1e8), { gasLimit: 1000000 });
  ...
  const handleUnStakeOk = async () => {
    try {
      if (isConnected) {
        ...
        const txn = await PoolContract.unStake(BigInt(unstakeData.Amount * 10 * 1e8), { gasLimit: 1000000 });
